...
class DonationApplication: MultiDexApplication() {
    ...
    override fun onCreate() {
        ...
        val omisePublicKey = bundle.getString("omise_api_public_key") ?: throw IllegalArgumentException("Omise public key not set in manifest!!")
        if (BuildConfig.DEBUG) {
            Timber.plant(Timber.DebugTree())
        }
        mApplicationComponent = DaggerApplicationComponent.builder()
            .applicationModule(
                ApplicationModule(baseUrl, omisePublicKey)
            )
            .build()
